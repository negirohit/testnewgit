package base;

import java.util.List;
import java.util.Set;

import org.openqa.selenium.By;
import org.openqa.selenium.JavascriptExecutor;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.FindAll;
import org.openqa.selenium.support.FindBy;
import org.openqa.selenium.support.How;
import org.openqa.selenium.support.events.EventFiringWebDriver;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.Select;
import org.openqa.selenium.support.ui.WebDriverWait;
import org.testng.Assert;
import org.xml.sax.Locator;



public class pageClass extends base
{
	public WebDriver driver;
	JavascriptExecutor js;
	WebDriverWait wait;
	//Homepage
	
	@FindBy(how=How.ID, using="wzrk-cancel")
	WebElement Clevertap;

	@FindBy(how=How.CSS, using=".ebela-sprite.logo") 
	WebElement EbelaLogo;
	
	@FindAll({@FindBy(css= "#menu-anchor")})
	public List<WebElement> SportsNav;
	
	@FindAll({@FindBy(css= ".trending_scrollar_list")})
	public List<WebElement> TrendingList;
	
	@FindBy(how=How.CSS, using=".fa.fa-facebook.fa-stack-1x")
	WebElement facebook;
	
	@FindBy(how=How.CSS, using=".fa.fa-twitter.fa-stack-1x")
	WebElement Twitter;
	
	@FindAll({@FindBy(css= ".black_footer_span2>a")})
	public List<WebElement> FooterPrivacy;
	
	@FindBy(how=How.CSS, using=".opinion_col.pos-rel>h3")
	public By pollwait;
	
	@FindAll({@FindBy(css= ".radio-inline>input")})
	public List<WebElement> radiobttn;
	
	
	@FindBy(how=How.CSS, using=".btn.btn-default.opinion_submit")
	WebElement submitpoll;
	
	@FindBy(how=How.XPATH, using=".//*[@id='opinoin_ques']/p")
	WebElement pollresult;
	
	@FindAll({@FindBy(css= ".Readerchoice>span")})
	public List<WebElement> curatedstories;
	
	@FindBy(how=How.CSS, using = "div.scroll-top-wrapper.show")
	WebElement BackToTopBttn ;
	
	@FindBy(how=How.CSS, using =".col-lg-12.col-lg-12.col-md-12.col-sm-12.col-xs-12.both0.testb")
	WebElement horoscope;
	
	@FindBy(how=How.CSS, using= ".at-h3.at-recommended-label")
	WebElement AddThis;
	
	@FindBy(how=How.XPATH, using=".//*[@id='pg_load_bd']/div/div[1]/div[3]/div[2]/div[1]/a")
	WebElement nanabidh;
	
	@FindAll({@FindBy(css= ".SecMisc>li")})
	public List<WebElement> nanabidh_list;
	
	@FindAll({@FindBy(css=".smprn")})
	public List<WebElement> SampurnaBibhag;
	
	@FindBy(how=How.XPATH, using="html/body/div[3]/div/div/div[1]/div/div/nav/div/div[2]/div/ul/li[9]/a")
	WebElement epaper;
	
	//Section Page
	@FindBy(how=How.XPATH, using= "html/body/div[15]/div/div/div[1]/div[2]/span[2]/a")
	WebElement privacysection;
	
	@FindBy(how=How.XPATH, using=".//*[@id='breadcrumb']/div/div/span[2]")
	WebElement secbreadcrumb;
	
	@FindBy(how=How.XPATH, using=".//*[@id='breadcrumb']/div/div/a/span")
	WebElement firstbreadcrumb;
	
	@FindBy(how=How.XPATH, using="html/body/div[5]/div/div/div/div[2]/ul/li[1]/a")
	WebElement SubsectionCrick;
	
	@FindBy(how=How.XPATH, using=".//*[@id='main']/div[20]/div[2]/a/span")
	WebElement LDMoreStory;
	
	
	//misc
	public void planner_page_wait()
	{
		wait = new WebDriverWait(this.driver, 60);
		wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.cssSelector(".addStory"))));
	}
	
	public void planner_explicit_wait(int i)
	{
		wait = new WebDriverWait(this.driver, i);
		wait.until(ExpectedConditions.visibilityOf(EbelaLogo));
	}
	
	public void page_scroll(int i)
	{
		js.executeScript("window.scrollBy(0,"+i+")");
	}
	
	
	//Homepage
	public void clevertap_click()
	{
		Clevertap.click();	
	}
	
	public void logo_click()
	{
		EbelaLogo.click();
	}
	
	public void sport_nav_click()
	{
		
		SportsNav.get(3).click();
	}
	
	public void trending_check(int i) throws Exception
	{
		
		TrendingList.get(i).click();
		
		
	}
	
	public void facebook_click()
	{
		facebook.click();
	}
	
	public void twitter_click()
	{
		
		Twitter.click();
	}
	
	public void windowhandles(WebDriver driver,String firstwindowhandle)
	{
		
		String secondwindowhandle = " ";
	    Set<String> windowhandles = driver.getWindowHandles();
	    
	    for(String id : windowhandles)
	    {
	    	
	    	if(!firstwindowhandle.equalsIgnoreCase(id))
	    		secondwindowhandle=id;
	    	
	    }
	    driver.switchTo().window(secondwindowhandle);
		
	}
	
	public void footer_click()
	{
		
		FooterPrivacy.get(1).click();
		
	}
	
	public void Poll_click(WebDriver driver) throws Exception
	{
		Thread.sleep(5000);
	    JavascriptExecutor jse = (JavascriptExecutor)driver;
	    	jse.executeScript("window.scrollBy(0,500)", "");
	    	
	    	Thread.sleep(3000);
	    	
		radiobttn.get(1).click();
		
		Thread.sleep(5000);
		
		submitpoll.click();
		
		Thread.sleep(10000);
		new WebDriverWait(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".qtn")));
		
		
		Assert.assertTrue(pollresult.isDisplayed());
		
	}
	
	public void BackToTop() throws Exception
	{
		
		
	    	Thread.sleep(2000);
	
	    	BackToTopBttn.click();
	}
	
	public int TotalCuratedStories()
	{
		
		int totalsize = curatedstories.size();

		return totalsize;
		
		
	}
	
	public void horoscope_click()
	{
		
		horoscope.click();
		
	}
	
	public String AddThis_check()
	{
		
		return AddThis.getText();
		
	}
	
	public void sampurna_bibhag_click()
	{
		SampurnaBibhag.get(1).click();
	}
	
	public void nanabidh_click()
	{
		
		nanabidh.click();
		nanabidh_list.get(1).click();
	}
	
	public void epaper_click()
	{
		epaper.click();
	}
	
	//Section
	public void privacysection_click()
	{
		privacysection.click();
	}
	
	public void secbreadcrumbcheck()
	{
		secbreadcrumb.click();
	}
	
	public void firstbreadcrumbcheck()
	{
		firstbreadcrumb.click();
	}
	
	public void subsection_click()
	{
		SubsectionCrick.click();
	}
	
	public String LoadMoreStoryClick()
	{
		if(LDMoreStory.isDisplayed())
		{
			return LDMoreStory.getText();
		}
		else
		{
			return null;
		}
	}
}
